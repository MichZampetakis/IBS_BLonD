...
class NagaitsevIBS():
    ...
    # ! ~~~~~~~~~~~~~~~~ Simple Kick ~~~~~~~~~~~~~~~~~~ !
    # ! ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ !
    def calculate_longitudinal_kick(self, emit_x, emit_y, beam, profile):
        index_alive = beam.id > 0
        Sig_zeta  = profile.bunchLength * c * self.betar / 4. #np.std(beam.dt[index_alive] * c * self.betar)
        print(profile.bunchLength * c * self.betar / 4., np.std(beam.dt[index_alive] * c * self.betar))
